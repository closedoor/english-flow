...
if (!process.env.PLAYWRIGHT_MODULE) throw new Error('Set PLAYWRIGHT_MODULE to the installed playwright/index.mjs path; see TESTING.md.');
...
for (const engine of ['chromium', 'webkit']) {
  ...
  async function check(name, run, initial = {}) {
    ...
    try {
      ...
      const detail = {engine,name,status:'FAIL',error:String(error),stack:error.stack,body:(await page.locator('body').innerText().catch(()=>'')).slice(0,3000),pageErrors:errors};
      results.push(detail);
  ...
  await check('quiz-header-no-overlap', async page => {
    await ready(page);
    const boxes = await page.locator('.quiz-page .compact-header').evaluate(header => [...header.children].map(child=>{const r=child.getBoundingClientRect();return {left:r.left,right:r.right,height:r.height,text:child.textContent};}));
    assert.ok(boxes[0].height<=56, `Pause label must not wrap vertically: ${JSON.stringify(boxes)}`);
...
console.log('BROWSER_SMOKE_SUMMARY',JSON.stringify({passed:results.filter(x=>x.status==='PASS').length,failed:results.filter(x=>x.status==='FAIL').length,total:results.length}));
